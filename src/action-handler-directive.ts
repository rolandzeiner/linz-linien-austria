...
interface ActionHandler extends HTMLElement {
  ...
  cancelled: boolean;
  held: boolean;
  // explicit `| undefined` (not `?:`) — under exactOptionalPropertyTypes
  // the optional shorthand rejects `field = undefined` assignments.
  timer: number | undefined;
  dblClickTimeout: number | undefined;
  isRepeating: boolean;
  repeatTimeout: number | undefined;
  ...
  actionHandler.bind = (element: ActionHandlerElement, options: ActionHandlerOptions = {}): void => {
    ...
    element.actionHandler.start = (ev: Event) => {
      ...
      if ((ev as TouchEvent).touches) {
        // noUncheckedIndexedAccess narrows touches[0] to Touch | undefined.
...
class ActionHandlerDirective extends Directive {
  // explicit `| undefined` so `previousOptions = options ? {...} : undefined`
  // assignment works under exactOptionalPropertyTypes.
  previousOptions: ActionHandlerOptions | undefined;
