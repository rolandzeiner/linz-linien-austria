...
* SPDX-License-Identifier: BSD-3-Clause
*/const G=Ae(class extends je{constructor(e){if(super(e),e.type!==ke.ATTRIBUTE||e.name!==`class`||e.strings?.length>2)throw Error("`classMap()` can only be used in the `class` attribute and must be the only part in the attribute.")}render(e){return` `+Object.keys(e).filter(t=>e[t]).join(` `)+` `}update(e,[t]){if(this.st===void 0){this.st=/* @__PURE__ */ new Set,e.strings!==void 0&&(this.nt=new Set(e.strings.join(` `).split(/\s/).filter(e=>e!==``)));for(let e in t)t[e]&&!this.nt?.has(e)&&this.st.add(e);return this.render(t)}let n=e.element.classList;for(let e of this.st)e in t||(n.remove(e),this.st.delete(e));for(let e in t){let r=!!t[e];r===this.st.has(e)||this.nt?.has(e)||(r?(n.add(e),this.st.add(e)):(n.remove(e),this.st.delete(e)))}return L}}),{I:Me}=be,Ne=e=>e,Pe=()=>document.createComment(``),K=(e,t,n)=>{
/**
* @license
...
`;function Z(e,t,n){e.dispatchEvent(new CustomEvent(t,{detail:n,bubbles:!0,composed:!0}))}function Q(e,t,n,r){var i=arguments.length,a=i<3?t:r===null?r=Object.getOwnPropertyDescriptor(t,n):r,o;if(typeof Reflect==`object`&&typeof Reflect.decorate==`function`)a=Reflect.decorate(e,t,n,r);else for(var s=e.length-1;s>=0;s--)(o=e[s])&&(a=(i<3?o(a):i>3?o(t,n,a):o(t,n))||a);return i>3&&a&&Object.defineProperty(t,n,a),a}let xt=class extends U{constructor(...e){super(...e),this._config={type:`linz-linien-austria-card`},this._computeLabel=e=>{let t=`ui.panel.lovelace.editor.card.generic.${e.name}`,n=this.hass?.localize?.(t);if(n)return n;let r=`editor.${e.name}`,i=J(r,{hassLanguage:this.hass?.language});return i===r?e.name:i},this._computeHelper=e=>{let t=`editor.${e.name}_helper`,n=J(t,{hassLanguage:this.hass?.language});return n===t?void 0:n},this._defaultOn=[`show_alerts`,`pulse_live`,`show_delay_colors`],this._onFormChanged=e=>{let t={...e.detail.value};for(let e of this._defaultOn)t[e]===!0&&delete t[e];this._config=t,Z(this,`config-changed`,{config:t})},this._onWalkTimeChange=(e,t)=>{let n=t.target.value.trim();if(n===``){this._patchRecord(`walk_times`,e,void 0);return}let r=Number(n);if(!Number.isFinite(r)||r<=0){this._patchRecord(`walk_times`,e,void 0);return}this._patchRecord(`walk_times`,e,Math.round(r))},this._onLineColorChange=(e,t)=>{let n=t.target;this._patchRecord(`line_colors`,e,n.value)},this._onLineColorClear=e=>{this._patchRecord(`line_colors`,e,void 0)}}setConfig(e){this._config={...e}}_t(e,t){return J(e,{hassLanguage:this.hass?.language},t)}_allKnownLines(){let e=/* @__PURE__ */ new Set,t=this._config.entity;if(t&&this.hass){let n=this.hass.states[t],r=n?.attributes?.lines_at_stop;if(Array.isArray(r))for(let t of r)typeof t==`string`&&t&&e.add(t);let i=n?.attributes?.departures;if(Array.isArray(i))for(let t of i)t.line&&e.add(t.line)}for(let t of this._config.lines??[])t&&e.add(t);return this._sortLines(Array.from(e))}_availableLines(){let e=(this._config.lines??[]).map(e=>e.trim()).filter(Boolean);return e.length>0?this._sortLines(e):this._allKnownLines()}_lineDestinations(){let e=/* @__PURE__ */ new Map,t=this._config.entity,n=t?this.hass?.states[t]?.attributes:void 0,r=n?.line_destinations;if(r&&typeof r==`object`&&!Array.isArray(r)){for(let[t,n]of Object.entries(r)){if(!n||typeof n!=`object`)continue;let r={};typeof n.H==`string`&&(r.H=n.H.trim()),typeof n.R==`string`&&(r.R=n.R.trim()),e.set(t,r)}return{destinations:e,fromRoster:!0}}let i=n?.departures;if(Array.isArray(i))for(let t of i){let n=t.dir_code;if(n!==`H`&&n!==`R`)continue;let r=(t.direction??``).trim();if(!r||!t.line)continue;let i=e.get(t.line)??{};i[n]===void 0&&(i[n]=r,e.set(t.line,i))}return{destinations:e,fromRoster:!1}}_onLineDirectionChange(e,t){let n={...this._config.line_directions??{}};t===null?delete n[e]:n[e]=t;let r={...this._config};Object.keys(n).length>0?r.line_directions=n:delete r.line_directions,this._config=r,Z(this,`config-changed`,{config:r})}_sortLines(e){return Array.from(new Set(e)).sort((e,t)=>{let n=parseInt(e,10),r=parseInt(t,10);return!Number.isNaN(n)&&!Number.isNaN(r)&&n!==r?n-r:e.localeCompare(t,void 0,{numeric:!0})})}_motForLine(e){let t=this._config.entity;if(!t||!this.hass)return;let n=this.hass.states[t]?.attributes?.departures;if(Array.isArray(n)){for(let t of n)if(t.line===e&&typeof t.mot==`number`)return t.mot}}_defaultColorForLine(e){return ot(this._motForLine(e))}_iconForLine(e){return at(this._motForLine(e))}_toggleLine(e){let t=new Set(this._config.lines??[]);t.has(e)?t.delete(e):t.add(e);let n={...this._config};t.size===0?delete n.lines:n.lines=this._sortLines(Array.from(t)),this._config=n,Z(this,`config-changed`,{config:n})}_onCustomLineSubmit(e){let t=e.value.trim();if(!t)return;let n=new Set(this._config.lines??[]);n.add(t);let r={...this._config,lines:this._sortLines(Array.from(n))};e.value=``,this._config=r,Z(this,`config-changed`,{config:r})}_renderLinesFilter(){let e=this._allKnownLines(),t=new Set(this._config.lines??[]);return I`
    ...
    `}_schema(){return[{name:`entity`,required:!0,selector:{entity:{filter:{domain:`sensor`,integration:`linz_linien_austria`}}}},{name:`name`,selector:{text:{}}},{name:`hide_header`,selector:{boolean:{}}},{name:`show_hero`,selector:{boolean:{}}},{name:`show_platform`,selector:{boolean:{}}},{name:`show_absolute_time`,selector:{boolean:{}}},{name:`show_delay_colors`,selector:{boolean:{}}},{name:`show_alerts`,selector:{boolean:{}}},{name:`pulse_live`,selector:{boolean:{}}},{name:`enable_animations`,selector:{boolean:{}}},{name:`max_departures`,selector:{number:{min:0,max:30,step:1,mode:`box`}}}]}_formData(){let e={...this._config};for(let t of this._defaultOn)e[t]===void 0&&(e[t]=!0);return e}_patchRecord(e,t,n){let r={...this._config[e]??{}};n==null||n===``?delete r[t]:r[t]=n;let i={...this._config};Object.keys(r).length===0?delete i[e]:i[e]=r,this._config=i,Z(this,`config-changed`,{config:i})}_renderPerLineSection(){let e=this._availableLines();if(e.length===0)return I`<div class="editor-hint">
        ${this._t(`editor.per_line_no_data`)}
      </div>`;let t=this._config.walk_times??{},n=this._config.line_colors??{},r=this._config.line_directions??{},{destinations:i,fromRoster:a}=this._lineDestinations();return I`
      <div class="editor-section">
        ...
        <div class="per-line-list">
          ${e.map(e=>{let o=t[e],s=n[e]??``,c=this._defaultColorForLine(e),l=s||c,u=r[e],d=i.get(e),f=e=>!a||d===void 0||e in d,p=f(`H`)&&f(`R`),m=(t,n)=>{let r=(u??null)===t,i,a;if(t===null){let e=this._t(`editor.direction_both`),t=d?.H||d?.R;i=p?d?.H&&d?.R?`${e}: ${d.H} ↔ ${d.R}`:e:t?this._t(`editor.direction_only_to`,{destination:t}):this._t(`editor.direction_one_way`),a=!p&&r}else{let e=this._t(t===`H`?`editor.direction_h`:`editor.direction_r`),n=d?.[t];i=f(t)?n?`${e} → ${n}`:e:this._t(`editor.direction_not_served`,{direction:e}),a=!f(t)&&!r}return I`<button
                type="button"
                class=${`per-line-dir${r?` is-active`:``}`}
                aria-pressed=${r}
                ?disabled=${a}
                title=${i}
                aria-label="${i} — ${this._t(`editor.line`)} ${e}"
              ...
              <div class="per-line-row">
                ...
                <button
                  class=${`per-line-clear${s?``:` is-hidden`}`}
                  type="button"
    ...
    `}_renderRow(e){let t=this._countdownFor(e),n=this.config.show_delay_colors!==!1,r=n&&typeof e.delay_minutes==`number`&&e.delay_minutes>0,i=n&&typeof e.delay_minutes==`number`&&e.delay_minutes<0,a=t===null?`—`:t<=0?this._t(`card.now`):`${t} ${this._t(`card.minutes_short`)}`,o=this.config.show_absolute_time&&!e.is_cancelled?this._clockTimeFor(e):null,s=e.is_cancelled?``:(e.delay_hint??``).trim(),c=e.is_cancelled?[]:e.stops_ahead??[],l=this._depKey(e),u=this._expandedStops.has(l),d=`stops-${this._slugify(l)}`,f=c.length>0,p=t!==null&&t<=0&&!e.is_cancelled,m=p?this._accentText(this._userLineColor(e.line)??ot(e.mot)):null,h=`${e.mot_name?`${e.mot_name} `:``}${e.line} ${e.direction} ${e.is_cancelled?this._t(`card.cancelled`):a}${o?`, ${this._t(`card.at_time`,{time:o})}`:``}${e.is_realtime?` ${this._t(`card.realtime`)}`:``}${s?`. ${s}`:``}`,g=f?`${h}. ${this._t(u?`card.hide_stops`:`card.show_stops`,{line:e.line,direction:e.direction})}`:h,_=I`
      <li class="row-wrap">
      <div
        class=${G({row:!0,"row-rt":!!e.is_realtime,"row-cancelled":!!e.is_cancelled,"row-expandable":f})}
        role=${f?`button`:R}
        tabindex=${f?`0`:R}
        aria-expanded=${f?u?`true`:`false`:R}
        aria-controls=${f?d:R}
        aria-label=${g}
        style=${m?`--linz-accent-text: ${m};`:R}
        @click=${()=>f&&this._toggleStops(l)}
        @keydown=${e=>this._onExpanderKeydown(e,f,()=>this._toggleStops(l))}
      >
        ${this._renderLineBadge(e)}
        <span class="row-main">
          <span class="row-direction">${e.direction||``}</span>
          ${s?I`<span class="row-hint" title=${s}>${s}</span>`:R}
        </span>
        <span class="row-tail">
          ${this.config.show_platform&&!e.is_cancelled&&this._platformText(e)?I`<span
              ...
              >`:R}
          ${o?I`<span class="row-clock" aria-hidden="true"
                >${o}</span
              >`:R}
          <span
            class=${G({"row-time":!0,late:r&&!e.is_cancelled,early:i&&!e.is_cancelled,now:p})}
          >
            ${e.is_cancelled?this._t(`card.cancelled`):a}
          </span>
          ${o&&!f?I`<span class="visually-hidden"
      ...
      >
        ${e.map((t,n)=>{let r=t.delay_minutes,i=n===e.length-1;return I`<li
          ...
          >
            ...
            <span
              class=${G({"stops-ahead-time":!0,late:this.config.show_delay_colors!==!1&&typeof r==`number`&&r>0,early:this.config.show_delay_colors!==!1&&typeof r==`number`&&r<0})}
