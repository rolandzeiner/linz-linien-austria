...
`;function it(e,t,i){e.dispatchEvent(new CustomEvent(t,{detail:i,bubbles:!0,composed:!0}))}let nt=class extends se{constructor(){super(...arguments),this._config={type:"linz-linien-austria-card"},this._computeLabel=e=>{const t=`ui.panel.lovelace.editor.card.generic.${e.name}`,i=this.hass?.localize?.(t);if(i)return i;const n=`editor.${e.name}`,r=Be(n,{hassLanguage:this.hass?.language});return r!==n?r:e.name},this._computeHelper=e=>{const t=`editor.${e.name}_helper`,i=Be(t,{hassLanguage:this.hass?.language});return i===t?void 0:i},this._onFormChanged=e=>{const t={...e.detail.value};this._config=t,it(this,"config-changed",{config:t})},this._onWalkTimeChange=(e,t)=>{const i=t.target.value.trim();if(""===i)return void this._patchRecord("walk_times",e,void 0);const n=Number(i);!Number.isFinite(n)||n<=0?this._patchRecord("walk_times",e,void 0):this._patchRecord("walk_times",e,Math.round(n))},this._onLineColorChange=(e,t)=>{const i=t.target;this._patchRecord("line_colors",e,i.value)},this._onLineColorClear=e=>{this._patchRecord("line_colors",e,void 0)}}setConfig(e){this._config={...e}}_t(e,t){return Be(e,{hassLanguage:this.hass?.language},t)}_allKnownLines(){const e=new Set,t=this._config.entity;if(t&&this.hass){const i=this.hass.states[t],n=i?.attributes?.lines_at_stop;if(Array.isArray(n))for(const t of n)"string"==typeof t&&t&&e.add(t);const r=i?.attributes?.departures;if(Array.isArray(r))for(const t of r)t.line&&e.add(t.line)}for(const t of this._config.lines??[])t&&e.add(t);return this._sortLines(Array.from(e))}_availableLines(){const e=(this._config.lines??[]).map(e=>e.trim()).filter(Boolean);return e.length>0?this._sortLines(e):this._allKnownLines()}_directionOptions(){const e=new Map,t=this._config.entity,i=t?this.hass?.states[t]?.attributes?.departures:void 0;if(Array.isArray(i))for(const t of i){const i=t.dir_code;if("H"!==i&&"R"!==i)continue;const n=(t.direction??"").trim();if(!n)continue;const r=e.get(i)??[];r.includes(n)||r.push(n),e.set(i,r)}return["H","R"].map(t=>{const i=this._t("H"===t?"editor.direction_h":"editor.direction_r"),n=[...e.get(t)??[]].sort((e,t)=>e.localeCompare(t,void 0,{numeric:!0})),r=n.slice(0,2).join(", ");return{value:t,label:n.length?`${r}${n.length>2?" …":""} (${i})`:i}})}_sortLines(e){return Array.from(new Set(e)).sort((e,t)=>{const i=parseInt(e,10),n=parseInt(t,10);return Number.isNaN(i)||Number.isNaN(n)||i===n?e.localeCompare(t,void 0,{numeric:!0}):i-n})}_motForLine(e){const t=this._config.entity;if(!t||!this.hass)return;const i=this.hass.states[t],n=i?.attributes?.departures;if(Array.isArray(n))for(const t of n)if(t.line===e&&"number"==typeof t.mot)return t.mot}_defaultColorForLine(e){return Ze(this._motForLine(e))}_iconForLine(e){return Ve(this._motForLine(e))}_toggleLine(e){const t=new Set(this._config.lines??[]);t.has(e)?t.delete(e):t.add(e);const i={...this._config};0===t.size?delete i.lines:i.lines=this._sortLines(Array.from(t)),this._config=i,it(this,"config-changed",{config:i})}_onCustomLineSubmit(e){const t=e.value.trim();if(!t)return;const i=new Set(this._config.lines??[]);i.add(t);const n={...this._config,lines:this._sortLines(Array.from(i))};e.value="",this._config=n,it(this,"config-changed",{config:n})}_renderLinesFilter(){const e=this._allKnownLines(),t=new Set(this._config.lines??[]);return B`
    ...
    `}static{this.styles=tt}};e([pe({attribute:!1})],nt.prototype,"hass",void 0),e([ue()],nt.prototype,"_config",void 0),nt=e([ce("linz-linien-austria-card-editor")],nt),window.customCards=window.customCards||[],window.customCards.push({type:"linz-linien-austria-card",name:"Linz Linien Austria",description:"Live LINZ AG LINIEN departure monitor.",preview:!0,documentationURL:"https://github.com/rolandzeiner/linz-linien-austria",getEntitySuggestion:(e,t)=>t.startsWith("sensor.")?"linz_linien_austria"!==e?.entities?.[t]?.platform?null:{config:{type:"custom:linz-linien-austria-card",entity:t,show_hero:!0}}:null});let rt=class extends se{constructor(){super(...arguments),this._versionMismatch=null,this._versionCheckDone=!1,this._expandedStops=new Set}static getConfigElement(){return document.createElement("linz-linien-austria-card-editor")}static getStubConfig(e){const t={show_hero:!0};if(!e)return t;const i=Object.keys(e.states).find(t=>{if(!t.startsWith("sensor."))return!1;const i=e.states[t]?.attributes;return void 0!==i&&"string"==typeof i.stop_id&&Array.isArray(i.departures)});return i&&(t.entity=i),t}setConfig(e){if(!e||"object"!=typeof e)throw new Error("Invalid configuration / Ungültige Konfiguration");this.config={show_hero:!0,...e}}_t(e,t){return Be(e,{configLanguage:this.config?.language,hassLanguage:this.hass?.language},t)}shouldUpdate(e){if(!this.config)return!1;if(e.has("config"))return!0;const t=e.get("hass");return!t||!!this.config.entity&&t.states[this.config.entity]!==this.hass.states[this.config.entity]}getCardSize(){return 6}getGridOptions(){return{columns:12,rows:"auto",min_columns:6,min_rows:4}}firstUpdated(){this._maybeRunVersionCheck()}updated(e){this._maybeRunVersionCheck()}_maybeRunVersionCheck(){!this._versionCheckDone&&this.hass&&(this._versionCheckDone=!0,async function(e){if(!e?.callWS)return null;try{const t=await e.callWS({type:"linz_linien_austria/card_version"});if(t?.version&&"1.0.0"!==t.version)return t.version}catch{}return null}(this.hass).then(e=>{e&&this.isConnected&&(this._versionMismatch=e)}))}render(){const e={configLanguage:this.config?.language,hassLanguage:this.hass?.language};if(!this.hass)return B`<ha-card><div class="card-content">…</div></ha-card>`;if(!this.config.entity)return B`<ha-card>
      ...
      </ha-card>`;const i=this.config.name||t.attributes.stop_name||t.attributes.friendly_name||"",n=t.attributes.latitude,r=t.attributes.longitude,o=t.attributes.stop_name||i,a="number"==typeof n&&"number"==typeof r?`${n},${r}`:o?encodeURIComponent(/Linz/i.test(o)?o:`${o}, Linz`):"",s=a?"string"!=typeof(l=`https://www.google.com/maps/search/?api=1&query=${a}`)?"":/^https?:\/\//i.test(l)?l:"":null;var l;const c=this._t("card.open_in_maps"),h=t.attributes.departures??[],d=new Set((this.config.lines??[]).map(e=>e.trim()).filter(Boolean)),p=this.config.directions,u=new Set((Array.isArray(p)?p:[]).map(e=>String(e).trim().toUpperCase()).filter(e=>"H"===e||"R"===e)),m=this.config.walk_times??{},f=h.filter(e=>{if(d.size>0&&!d.has(e.line))return!1;if(u.size>0&&e.dir_code&&!u.has(e.dir_code))return!1;const t=m[e.line];if("number"==typeof t&&t>0){const i=this._countdownFor(e);if(null===i||i<t)return!1}return!0}),g="number"==typeof this.config.max_departures?Math.max(0,this.config.max_departures):f.length,_=this._computeHeroGroup(f),w=_[0],b=!1!==this.config.show_hero?new Set(_):new Set,v=f.filter(e=>!b.has(e)),y=0===g?[]:v.slice(0,g),x=Ve(w?.mot),$=this._userLineColor(w?.line)??Ge(w?.mot),k=this._accentText(this._userLineColor(w?.line)??Ze(w?.mot)),A=($?`--header-color: ${$};`:"")+(k?`--header-text: ${k};`:""),z=new Set(f.map(e=>e.line).filter(Boolean)),S=(t.attributes.alerts??[]).filter(e=>{const t=e.affected_lines||[];return 0===t.length||t.some(e=>z.has(e))}),C=w?.direction||"",E=this.config.show_platform?this._platformText(w):"",L=E?`${C} · ${this._platformLabel(w,!0)} ${E}`:C,T=!1!==this.config.pulse_live,M=!!this.config.enable_animations;return B`
      <ha-card
        class=${we({"no-pulse":!T,"with-animations":M})}
      >
        ${Qe(this._versionMismatch,e)}
        ${this.config.hide_header?K:B`<header class="head" style=${A}>
              <span class="icon-tile" aria-hidden="true">
                <ha-icon icon=${x}></ha-icon>
              </span>
              <div class="title-block">
                <h3 class="title">${i}</h3>
                ${L?B`<p class="subtitle">${L}</p>`:K}
              </div>
        ...
        ${!1!==this.config.show_alerts&&S.length>0?this._renderAlerts(S):K}
        ${this.config.show_hero&&_.length>0?this._renderHero(_):K}
        ${0===g||0===y.length&&b.size>0?K:B`<ul class="departures" role="list">
                ${0===y.length?B`<li class="empty">
                      ${(d.size>0||u.size>0)&&h.length>0?this._t("card.no_matches_for_filter"):this._t("card.no_departures")}
                    </li>`:Ce(y,e=>this._depKey(e),e=>this._renderRow(e))}
              </ul>`}
        <div class="foot">
      ...
      <section class="alerts" role="region" aria-label=${i}>
        <details>
          ...
          <ul class="alerts-list" role="list">
            ${t.map(e=>B`
                <li
                  class=${we({alert:!0,"alert-high":this._isHighPriority(e.priority)})}
                >
                  <div class="alert-title">${e.title}</div>
                  ${e.description?B`<div class="alert-body">${e.description}</div>`:K}
    ...
    `}_computeHeroGroup(e){if(0===e.length)return[];const t=e.filter(e=>!e.is_cancelled);if(0===t.length){const t=e[0];return t?[t]:[]}const i=e=>"number"==typeof e.countdown_rt?e.countdown_rt:"number"==typeof e.countdown?e.countdown:Number.POSITIVE_INFINITY;let n=Number.POSITIVE_INFINITY;for(const e of t){const t=i(e);t<n&&(n=t)}if(!Number.isFinite(n)){const e=t[0];return e?[e]:[]}return n<=0?t.filter(e=>i(e)<=0):t.filter(e=>i(e)===n)}_renderHero(e){const t=e[0],i=this._countdownFor(t),n=t.is_cancelled?this._t("card.cancelled"):null===i?"—":i<=0?this._t("card.now"):`${i}`,r=e.map(e=>`${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction}`),o=t.is_cancelled?this._t("card.cancelled"):null===i?this._t("card.unknown"):i<=0?this._t("card.now"):`${i} ${this._t("card.minutes")}`,a=this.config.show_absolute_time&&!t.is_cancelled&&1===e.length?this._clockTimeFor(t):null,s=` ${this._t("card.and_separator")} `,l=`${this._t("card.next_departure_label")}: ${r.join(s)}, ${o}${a?`, ${this._t("card.at_time",{time:a})}`:""}${t.is_realtime&&!t.is_cancelled?`, ${this._t("card.realtime")}`:""}`,c=t.is_cancelled?null:this._userLineColor(t.line)??Ge(t.mot),h=t.is_cancelled?null:this._accentText(this._userLineColor(t.line)??Ze(t.mot)),d=(c?`--hero-color: ${c};`:"")+(h?`--hero-text: ${h};`:"");return B`
      <section
        class=${we({hero:!0,"hero-cancelled":!!t.is_cancelled,"hero-multi":e.length>1})}
        aria-label=${l}
        style=${d}
      >
        <div class="hero-time">
          ...
          ${!t.is_cancelled&&null!==i&&i>0?B`<span class="hero-unit"
              ...
              >`:K}
          ${a?B`<span class="hero-clock" aria-hidden="true"
                >${a}</span
              >`:K}
        </div>
    ...
    `}_renderHeroEntry(e){const t=this.config.show_platform?this._platformText(e):"",i=e.is_cancelled?"":(e.delay_hint??"").trim(),n=(e.is_cancelled?[]:e.stops_ahead??[]).length>0,r=this._depKey(e),o=this._expandedStops.has(r),a=`hero-stops-${this._slugify(r)}`,s=`${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction}`,l=n?`${s}. ${this._t(o?"card.hide_stops":"card.show_stops",{line:e.line,direction:e.direction})}`:s;return B`
      <div
        class=${we({"hero-entry":!0,"hero-entry-expandable":n,expanded:o})}
        role=${n?"button":K}
        tabindex=${n?"0":K}
      ...
      >
        ...
        ${n?B`<ha-icon
              ...
              aria-hidden="true"
            ></ha-icon>`:K}
        ${i?B`<span class="hero-hint">${i}</span>`:K}
      </div>
    `}_renderHeroStops(e){const t=e.is_cancelled?[]:e.stops_ahead??[];if(0===t.length)return K;const i=this._depKey(e),n=this._expandedStops.has(i),r=`hero-stops-${this._slugify(i)}`,o=`${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction}`;return B`
      <div class=${we({"hero-detail":!0,expanded:n})}>
        <div
    ...
    `}_renderRow(e){const t=this._countdownFor(e),i="number"==typeof e.delay_minutes&&e.delay_minutes>0,n="number"==typeof e.delay_minutes&&e.delay_minutes<0,r=null===t?"—":t<=0?this._t("card.now"):`${t} ${this._t("card.minutes_short")}`,o=this.config.show_absolute_time&&!e.is_cancelled?this._clockTimeFor(e):null,a=e.is_cancelled?"":(e.delay_hint??"").trim(),s=e.is_cancelled?[]:e.stops_ahead??[],l=this._depKey(e),c=this._expandedStops.has(l),h=`stops-${this._slugify(l)}`,d=s.length>0,p=null!==t&&t<=0&&!e.is_cancelled,u=p?this._accentText(this._userLineColor(e.line)??Ze(e.mot)):null,m=`${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction} ${e.is_cancelled?this._t("card.cancelled"):r}${o?`, ${this._t("card.at_time",{time:o})}`:""}${e.is_realtime?` ${this._t("card.realtime")}`:""}${a?`. ${a}`:""}`,f=d?`${m}. ${this._t(c?"card.hide_stops":"card.show_stops",{line:e.line,direction:e.direction})}`:m,g=B`
      <li class="row-wrap">
      <div
        class=${we({row:!0,"row-rt":!!e.is_realtime,"row-cancelled":!!e.is_cancelled,"row-expandable":d})}
        role=${d?"button":K}
        tabindex=${d?"0":K}
        aria-expanded=${d?c?"true":"false":K}
        aria-controls=${d?h:K}
        aria-label=${f}
        style=${u?`--linz-accent-text: ${u};`:K}
        @click=${()=>d&&this._toggleStops(l)}
        @keydown=${e=>this._onExpanderKeydown(e,d,()=>this._toggleStops(l))}
      >
        ${this._renderLineBadge(e)}
        <span class="row-main">
          <span class="row-direction">${e.direction||""}</span>
          ${a?B`<span class="row-hint" title=${a}>${a}</span>`:K}
        </span>
        <span class="row-tail">
          ${this.config.show_platform&&!e.is_cancelled&&this._platformText(e)?B`<span
              ...
              >`:K}
          <span
            class=${we({"row-time":!0,late:i&&!e.is_cancelled,early:n&&!e.is_cancelled,now:p})}
          >
            ${e.is_cancelled?this._t("card.cancelled"):r}
          </span>
          ${o?B`<span class="row-clock" aria-hidden="true"
                  >${o}</span
                >${d?K:B`<span class="visually-hidden"
                      >${this._t("card.at_time",{time:o})}</span
                    >`}`:K}
          ${d?B`<ha-icon
                class="row-chevron"
                icon="mdi:chevron-down"
    ...
    `;return 0===s.length?g:[g,this._renderStopsAheadPanel(s,h,c,e)]}_onExpanderKeydown(e,t,i){t&&("Enter"!==e.key&&" "!==e.key||(e.preventDefault(),i()))}_toggleStops(e){const t=new Set(this._expandedStops);t.delete(e)||t.add(e),this._expandedStops=t}_slugify(e){return e.replace(/[^a-zA-Z0-9_-]+/g,"-")}_renderStopsAheadPanel(e,t,i,n){const r=`${n.mot_name?`${n.mot_name} `:""}${n.line} ${n.direction}`;return B`
      <li class=${we({"row-detail":!0,expanded:i})}>
        <div
    ...
    `}_renderStopsAheadTrail(e,t){const i=this._userLineColor(t.line)??Ge(t.mot)??"var(--linz-accent)";return B`
      <ol
        class="stops-ahead"
        style=${Te({"--stops-ahead-line":i})}
      >
        ${e.map((t,i)=>{const n=t.delay_minutes,r=i===e.length-1;return B`<li
            class=${we({"stops-ahead-stop":!0,terminus:r})}
          >
            <span class="stops-ahead-dot" aria-hidden="true"></span>
            <span class="stops-ahead-name">${t.name}</span>
            <span
              class=${we({"stops-ahead-time":!0,late:"number"==typeof n&&n>0,early:"number"==typeof n&&n<0})}
              >${this._clockTime(t.arrival)}</span
            >
          </li>`})}
      </ol>
    `}_clockTime(e){return!e||e.length<16?"":e.slice(11,16)}_renderLineBadge(e){const t=Ve(e.mot,"mdi:bus"),i=this._userLineColor(e.line),n=i?`background: ${i};`:"";return B`
      <span
        class="line-badge"
        data-mot=${e.mot??""}
        style=${n}
