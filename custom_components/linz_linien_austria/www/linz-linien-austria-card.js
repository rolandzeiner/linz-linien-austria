...
`;function qe(e,t,i){e.dispatchEvent(new CustomEvent(t,{detail:i,bubbles:!0,composed:!0}))}let Ze=class extends se{constructor(){super(...arguments),this._config={type:"linz-linien-austria-card"},this._computeLabel=e=>{const t=`ui.panel.lovelace.editor.card.generic.${e.name}`,i=this.hass?.localize?.(t);if(i)return i;const r=`editor.${e.name}`,n=je(r,{hassLanguage:this.hass?.language});return n!==r?n:e.name},this._computeHelper=e=>{const t=`editor.${e.name}_helper`,i=je(t,{hassLanguage:this.hass?.language});return i===t?void 0:i},this._onFormChanged=e=>{const t={...e.detail.value};this._config=t,qe(this,"config-changed",{config:t})},this._onWalkTimeChange=(e,t)=>{const i=t.target.value.trim();if(""===i)return void this._patchRecord("walk_times",e,void 0);const r=Number(i);!Number.isFinite(r)||r<=0?this._patchRecord("walk_times",e,void 0):this._patchRecord("walk_times",e,Math.round(r))},this._onLineColorChange=(e,t)=>{const i=t.target;this._patchRecord("line_colors",e,i.value)},this._onLineColorClear=e=>{this._patchRecord("line_colors",e,void 0)}}setConfig(e){this._config={...e}}_t(e,t){return je(e,{hassLanguage:this.hass?.language},t)}_allKnownLines(){const e=new Set,t=this._config.entity;if(t&&this.hass){const i=this.hass.states[t],r=i?.attributes?.lines_at_stop;if(Array.isArray(r))for(const t of r)"string"==typeof t&&t&&e.add(t);const n=i?.attributes?.departures;if(Array.isArray(n))for(const t of n)t.line&&e.add(t.line)}for(const t of this._config.lines??[])t&&e.add(t);return this._sortLines(Array.from(e))}_availableLines(){const e=(this._config.lines??[]).map(e=>e.trim()).filter(Boolean);return e.length>0?this._sortLines(e):this._allKnownLines()}_sortLines(e){return Array.from(new Set(e)).sort((e,t)=>{const i=parseInt(e,10),r=parseInt(t,10);return Number.isNaN(i)||Number.isNaN(r)||i===r?e.localeCompare(t,void 0,{numeric:!0}):i-r})}_motForLine(e){const t=this._config.entity;if(!t||!this.hass)return;const i=this.hass.states[t],r=i?.attributes?.departures;if(Array.isArray(r))for(const t of r)if(t.line===e&&"number"==typeof t.mot)return t.mot}_defaultColorForLine(e){return We(this._motForLine(e))??"#f08000"}_iconForLine(e){return Fe(this._motForLine(e))}_toggleLine(e){const t=new Set(this._config.lines??[]);t.has(e)?t.delete(e):t.add(e);const i={...this._config};0===t.size?delete i.lines:i.lines=this._sortLines(Array.from(t)),this._config=i,qe(this,"config-changed",{config:i})}_onCustomLineSubmit(e){const t=e.value.trim();if(!t)return;const i=new Set(this._config.lines??[]);i.add(t);const r={...this._config,lines:this._sortLines(Array.from(i))};e.value="",this._config=r,qe(this,"config-changed",{config:r})}_renderLinesFilter(){const e=this._allKnownLines(),t=new Set(this._config.lines??[]);return F`
    ...
    `}_renderHeroEntry(e){const t=this.config.show_platform?this._platformText(e):"";return F`
    ...
    `}_renderRow(e){const t=this._countdownFor(e),i="number"==typeof e.delay_minutes&&e.delay_minutes>0,r="number"==typeof e.delay_minutes&&e.delay_minutes<0,n=null===t?"—":t<=0?this._t("card.now"):`${t} ${this._t("card.minutes_short")}`;return F`
      <li
    ...
    `}_renderLineBadge(e){const t=Fe(e.mot,"mdi:bus"),i=this._userLineColor(e.line),r=i?`background: ${i};`:"";return F`
      <span
        class="line-badge"
        data-mot=${e.mot??""}
        style=${r}
