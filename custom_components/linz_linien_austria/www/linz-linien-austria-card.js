...
`;function qe(e,t,i){e.dispatchEvent(new CustomEvent(t,{detail:i,bubbles:!0,composed:!0}))}let Ze=class extends se{constructor(){super(...arguments),this._config={type:"linz-linien-austria-card"},this._computeLabel=e=>{const t=`ui.panel.lovelace.editor.card.generic.${e.name}`,i=this.hass?.localize?.(t);if(i)return i;const n=`editor.${e.name}`,r=je(n,{hassLanguage:this.hass?.language});return r!==n?r:e.name},this._computeHelper=e=>{const t=`editor.${e.name}_helper`,i=je(t,{hassLanguage:this.hass?.language});return i===t?void 0:i},this._onFormChanged=e=>{const t={...e.detail.value};this._config=t,qe(this,"config-changed",{config:t})},this._onWalkTimeChange=(e,t)=>{const i=t.target.value.trim();if(""===i)return void this._patchRecord("walk_times",e,void 0);const n=Number(i);!Number.isFinite(n)||n<=0?this._patchRecord("walk_times",e,void 0):this._patchRecord("walk_times",e,Math.round(n))},this._onLineColorChange=(e,t)=>{const i=t.target;this._patchRecord("line_colors",e,i.value)},this._onLineColorClear=e=>{this._patchRecord("line_colors",e,void 0)}}setConfig(e){this._config={...e}}_t(e,t){return je(e,{hassLanguage:this.hass?.language},t)}_allKnownLines(){const e=new Set,t=this._config.entity;if(t&&this.hass){const i=this.hass.states[t],n=i?.attributes?.lines_at_stop;if(Array.isArray(n))for(const t of n)"string"==typeof t&&t&&e.add(t);const r=i?.attributes?.departures;if(Array.isArray(r))for(const t of r)t.line&&e.add(t.line)}for(const t of this._config.lines??[])t&&e.add(t);return this._sortLines(Array.from(e))}_availableLines(){const e=(this._config.lines??[]).map(e=>e.trim()).filter(Boolean);return e.length>0?this._sortLines(e):this._allKnownLines()}_sortLines(e){return Array.from(new Set(e)).sort((e,t)=>{const i=parseInt(e,10),n=parseInt(t,10);return Number.isNaN(i)||Number.isNaN(n)||i===n?e.localeCompare(t,void 0,{numeric:!0}):i-n})}_motForLine(e){const t=this._config.entity;if(!t||!this.hass)return;const i=this.hass.states[t],n=i?.attributes?.departures;if(Array.isArray(n))for(const t of n)if(t.line===e&&"number"==typeof t.mot)return t.mot}_defaultColorForLine(e){return We(this._motForLine(e))??"#f08000"}_iconForLine(e){return Fe(this._motForLine(e))}_toggleLine(e){const t=new Set(this._config.lines??[]);t.has(e)?t.delete(e):t.add(e);const i={...this._config};0===t.size?delete i.lines:i.lines=this._sortLines(Array.from(t)),this._config=i,qe(this,"config-changed",{config:i})}_onCustomLineSubmit(e){const t=e.value.trim();if(!t)return;const i=new Set(this._config.lines??[]);i.add(t);const n={...this._config,lines:this._sortLines(Array.from(i))};e.value="",this._config=n,qe(this,"config-changed",{config:n})}_renderLinesFilter(){const e=this._allKnownLines(),t=new Set(this._config.lines??[]);return F`
    ...
    `}_renderAlerts(e){const t=[...e].sort((e,t)=>("high"===e.priority?0:1)-("high"===t.priority?0:1)),i=this._t("card.alerts_summary",{count:t.length});return F`
          ...
          <ul class="alerts-list" role="list">
            ${t.map(e=>F`
                ...
                >
                  ...
                  ${e.description?F`<div class="alert-body">${e.description}</div>`:V}
    ...
    `}_computeHeroGroup(e){if(0===e.length)return[];const t=e.filter(e=>!e.is_cancelled);if(0===t.length){const t=e[0];return t?[t]:[]}const i=e=>"number"==typeof e.countdown_rt?e.countdown_rt:"number"==typeof e.countdown?e.countdown:Number.POSITIVE_INFINITY;let n=Number.POSITIVE_INFINITY;for(const e of t){const t=i(e);t<n&&(n=t)}if(!Number.isFinite(n)){const e=t[0];return e?[e]:[]}return n<=0?t.filter(e=>i(e)<=0):t.filter(e=>i(e)===n)}_renderHero(e){const t=e[0],i=this._countdownFor(t),n=t.is_cancelled?this._t("card.cancelled"):null===i?"—":i<=0?this._t("card.now"):`${i}`,r=e.map(e=>`${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction}`),o=t.is_cancelled?this._t("card.cancelled"):null===i?this._t("card.unknown"):i<=0?this._t("card.now"):`${i} ${this._t("card.minutes")}`,a=` ${this._t("card.and_separator")} `,s=`${this._t("card.next_departure_label")}: ${r.join(a)}, ${o}${t.is_realtime&&!t.is_cancelled?`, ${this._t("card.realtime")}`:""}`,l=this._userLineColor(t.line)??We(t.mot),c=l?`--hero-color: ${l};`:"";return F`
      ...
      >
        ...
        <div class="hero-meta">
          ${Ce(e,e=>this._depKey(e),e=>this._renderHeroEntry(e))}
        </div>
      </section>
    `}_renderHeroEntry(e){const t=this.config.show_platform?this._platformText(e):"",i=e.is_cancelled?"":(e.delay_hint??"").trim();return F`
      <div class="hero-entry">
        ...
        ${i?F`<span class="hero-hint">${i}</span>`:V}
      </div>
    `}_renderRow(e){const t=this._countdownFor(e),i="number"==typeof e.delay_minutes&&e.delay_minutes>0,n="number"==typeof e.delay_minutes&&e.delay_minutes<0,r=null===t?"—":t<=0?this._t("card.now"):`${t} ${this._t("card.minutes_short")}`,o=e.is_cancelled?"":(e.delay_hint??"").trim(),a=e.is_cancelled?[]:e.stops_ahead??[],s=this._depKey(e),l=this._expandedStops.has(s),c=`stops-${this._slugify(s)}`;return F`
      <li
        class=${ve({row:!0,"row-rt":!!e.is_realtime,"row-cancelled":!!e.is_cancelled,"row-expandable":a.length>0})}
        aria-label="${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction} ${e.is_cancelled?this._t("card.cancelled"):r}${e.is_realtime?` ${this._t("card.realtime")}`:""}${o?`. ${o}`:""}"
      >
        ${this._renderLineBadge(e)}
        <span class="row-main">
          <span class="row-direction">${e.direction||""}</span>
          ${o?F`<span class="row-hint" title=${o}>${o}</span>`:V}
        </span>
        <span class="row-tail">
          ...
          ${a.length>0?F`<button
                ...
                aria-controls=${c}
                aria-label=${this._t(l?"card.hide_stops":"card.show_stops",{line:e.line,direction:e.direction})}
                title=${this._t(l?"card.hide_stops":"card.show_stops",{line:e.line,direction:e.direction})}
                @click=${()=>this._toggleStops(s)}
              >
                <ha-icon
        ...
        ${l&&a.length>0?this._renderStopsAhead(a,c):V}
      </li>
    `}_toggleStops(e){const t=new Set(this._expandedStops);t.delete(e)||t.add(e),this._expandedStops=t}_slugify(e){return e.replace(/[^a-zA-Z0-9_-]+/g,"-")}_renderStopsAhead(e,t){return F`
      <ul class="stops-ahead" id=${t}>
        ${e.map(e=>{const t=e.delay_minutes,i="number"==typeof t&&t>0,n="number"==typeof t&&t<0;return F`<li class="stop-ahead">
            <span class="stop-name">${e.name}</span>
            <span
              class=${ve({"stop-time":!0,late:i,early:n})}
              >${this._clockTime(e.arrival)}</span
            >
          </li>`})}
      </ul>
    `}_clockTime(e){return!e||e.length<16?"":e.slice(11,16)}_renderLineBadge(e){const t=Fe(e.mot,"mdi:bus"),i=this._userLineColor(e.line),n=i?`background: ${i};`:"";return F`
      <span
        class="line-badge"
        data-mot=${e.mot??""}
        style=${n}
    ...
    `}_userLineColor(e){if(!e)return null;const t=this.config.line_colors??{};return t[e]??t[e.toUpperCase()]??null}_depKey(e){return[e.line??"",e.direction??"",e.platform??"",e.scheduled??""].join("|")}_platformText(e){if(!e)return"";const t=(e.platform??"").trim();return t&&"0"!==t?t:""}_isRailMot(e){return 0===e||1===e}_platformLabel(e,t){const i=this._isRailMot(e?.mot)?t?"card.platform_rail_short":"card.platform_rail":t?"card.platform_short":"card.platform";return this._t(i)}_countdownFor(e){return"number"==typeof e.countdown_rt?e.countdown_rt:"number"==typeof e.countdown?e.countdown:null}static{this.styles=Ke}};e([pe({attribute:!1})],Ye.prototype,"hass",void 0),e([ue()],Ye.prototype,"config",void 0),e([ue()],Ye.prototype,"_versionMismatch",void 0),e([ue()],Ye.prototype,"_expandedStops",void 0),Ye=e([ce("linz-linien-austria-card")],Ye);export{Ye as LinzLinienAustriaCard};
