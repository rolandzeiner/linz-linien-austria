...
`;function Je(e,t,i){e.dispatchEvent(new CustomEvent(t,{detail:i,bubbles:!0,composed:!0}))}let Xe=class extends se{constructor(){super(...arguments),this._config={type:"linz-linien-austria-card"},this._computeLabel=e=>{const t=`ui.panel.lovelace.editor.card.generic.${e.name}`,i=this.hass?.localize?.(t);if(i)return i;const r=`editor.${e.name}`,n=Fe(r,{hassLanguage:this.hass?.language});return n!==r?n:e.name},this._computeHelper=e=>{const t=`editor.${e.name}_helper`,i=Fe(t,{hassLanguage:this.hass?.language});return i===t?void 0:i},this._onFormChanged=e=>{const t={...e.detail.value};this._config=t,Je(this,"config-changed",{config:t})},this._onWalkTimeChange=(e,t)=>{const i=t.target.value.trim();if(""===i)return void this._patchRecord("walk_times",e,void 0);const r=Number(i);!Number.isFinite(r)||r<=0?this._patchRecord("walk_times",e,void 0):this._patchRecord("walk_times",e,Math.round(r))},this._onLineColorChange=(e,t)=>{const i=t.target;this._patchRecord("line_colors",e,i.value)},this._onLineColorClear=e=>{this._patchRecord("line_colors",e,void 0)}}setConfig(e){this._config={...e}}_t(e,t){return Fe(e,{hassLanguage:this.hass?.language},t)}_allKnownLines(){const e=new Set,t=this._config.entity;if(t&&this.hass){const i=this.hass.states[t],r=i?.attributes?.lines_at_stop;if(Array.isArray(r))for(const t of r)"string"==typeof t&&t&&e.add(t);const n=i?.attributes?.departures;if(Array.isArray(n))for(const t of n)t.line&&e.add(t.line)}for(const t of this._config.lines??[])t&&e.add(t);return this._sortLines(Array.from(e))}_availableLines(){const e=(this._config.lines??[]).map(e=>e.trim()).filter(Boolean);return e.length>0?this._sortLines(e):this._allKnownLines()}_sortLines(e){return Array.from(new Set(e)).sort((e,t)=>{const i=parseInt(e,10),r=parseInt(t,10);return Number.isNaN(i)||Number.isNaN(r)||i===r?e.localeCompare(t,void 0,{numeric:!0}):i-r})}_motForLine(e){const t=this._config.entity;if(!t||!this.hass)return;const i=this.hass.states[t],r=i?.attributes?.departures;if(Array.isArray(r))for(const t of r)if(t.line===e&&"number"==typeof t.mot)return t.mot}_defaultColorForLine(e){return Ge(this._motForLine(e))??"#f08000"}_iconForLine(e){return Ve(this._motForLine(e))}_toggleLine(e){const t=new Set(this._config.lines??[]);t.has(e)?t.delete(e):t.add(e);const i={...this._config};0===t.size?delete i.lines:i.lines=this._sortLines(Array.from(t)),this._config=i,Je(this,"config-changed",{config:i})}_onCustomLineSubmit(e){const t=e.value.trim();if(!t)return;const i=new Set(this._config.lines??[]);i.add(t);const r={...this._config,lines:this._sortLines(Array.from(i))};e.value="",this._config=r,Je(this,"config-changed",{config:r})}_renderLinesFilter(){const e=this._allKnownLines(),t=new Set(this._config.lines??[]);return F`
    ...
    `}_renderHeroEntry(e){const t=this.config.show_platform?this._platformText(e):"",i=e.is_cancelled?"":(e.delay_hint??"").trim(),r=(e.is_cancelled?[]:e.stops_ahead??[]).length>0,n=this._depKey(e),o=this._expandedStops.has(n),a=`hero-stops-${this._slugify(n)}`,s=`${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction}`,l=r?`${s}. ${this._t(o?"card.hide_stops":"card.show_stops",{line:e.line,direction:e.direction})}`:s;return F`
      <div
        class=${ve({"hero-entry":!0,"hero-entry-expandable":r,expanded:o})}
        role=${r?"button":K}
        tabindex=${r?"0":K}
    ...
    `}_renderHeroStops(e){const t=e.is_cancelled?[]:e.stops_ahead??[];if(0===t.length)return K;const i=this._depKey(e),r=this._expandedStops.has(i),n=`hero-stops-${this._slugify(i)}`,o=`${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction}`;return F`
      <div class=${ve({"hero-detail":!0,expanded:r})}>
        <div
    ...
    `}_renderRow(e){const t=this._countdownFor(e),i="number"==typeof e.delay_minutes&&e.delay_minutes>0,r="number"==typeof e.delay_minutes&&e.delay_minutes<0,n=null===t?"—":t<=0?this._t("card.now"):`${t} ${this._t("card.minutes_short")}`,o=e.is_cancelled?"":(e.delay_hint??"").trim(),a=e.is_cancelled?[]:e.stops_ahead??[],s=this._depKey(e),l=this._expandedStops.has(s),d=`stops-${this._slugify(s)}`,c=a.length>0,h=`${e.mot_name?`${e.mot_name} `:""}${e.line} ${e.direction} ${e.is_cancelled?this._t("card.cancelled"):n}${e.is_realtime?` ${this._t("card.realtime")}`:""}${o?`. ${o}`:""}`,p=c?`${h}. ${this._t(l?"card.hide_stops":"card.show_stops",{line:e.line,direction:e.direction})}`:h,u=F`
      <li class="row-wrap">
      <div
        class=${ve({row:!0,"row-rt":!!e.is_realtime,"row-cancelled":!!e.is_cancelled,"row-expandable":c})}
        role=${c?"button":K}
        tabindex=${c?"0":K}
    ...
    `}_renderStopsAheadTrail(e,t){const i=this._userLineColor(t.line)??Ge(t.mot)??"var(--linz-accent)";return F`
      ...
      >
        ${e.map((t,i)=>{const r=t.delay_minutes,n=i===e.length-1;return F`<li
            class=${ve({"stops-ahead-stop":!0,terminus:n})}
          >
            <span class="stops-ahead-dot" aria-hidden="true"></span>
            <span class="stops-ahead-name">${t.name}</span>
            <span
              class=${ve({"stops-ahead-time":!0,late:"number"==typeof r&&r>0,early:"number"==typeof r&&r<0})}
