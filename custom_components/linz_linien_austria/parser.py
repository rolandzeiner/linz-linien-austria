"""Pure parsing + normalisation of the LINZ AG LINIEN EFA JSON payloads.

This module never touches the network. It turns the raw ``dict`` bodies
that :mod:`.api` fetches into the flat, normalised shapes the coordinator
and the card consume — stop candidates, resolved-stop metadata, departure
rows, onward stop sequences and the stop's line roster.

Keeping it separate from :mod:`.api` (which owns the HTTP transport and
request-parameter assembly) means the parsers can be unit-tested against
...
from .const import MAX_STOPS_AHEAD
...
def _wgs84_from_coords(raw: Any) -> tuple[float, float] | None:
    ...
    Every request the client makes sends
